...
class NameValidator:
    ...
    def __call__(self, value):
        ...
        if not value.isalpha():
            errors.append(
                f"{self.field_name} should only contain alphabetic characters."
            )
        if errors:
            raise serializers.ValidationError(errors)
...
class UsernameValidator:
    def __call__(self, value):
        ...
        if value[0] in "_-" or value[-1] in "_-":
            errors.append(
                "Username cannot start or end with an underscore, hyphen, or dot."
            )
        if any(bad in value for bad in ["__", "--", "_-", "-_"]):
            errors.append(
                "Username cannot contain consecutive special characters like '__', '_-', or similar."
            )
